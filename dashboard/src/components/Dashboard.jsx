...
const Dashboard = () => {
	...
	useEffect(() => {
		const fetchAppointments = async () => {
			try {
				const { data } = await axios.get("https://hms-backend-deployment-y8nl.onrender.com/api/v1/appointment/getall", { withCredentials: true });
	...
	const handleUpdateStatus = async (appointmentId, status) => {
		try {
			const { data } = await axios.put(`https://hms-backend-deployment-y8nl.onrender.com/api/v1/appointment/update/${appointmentId}`, { status }, { withCredentials: true });
				...
				<div className="banner">
					<div className="firstBox">
						...
						<div className="content">
							...
							<p>Wellcome to ZeeCare Medical Institue's Admin Dashboard</p>
						</div>
					</div>
