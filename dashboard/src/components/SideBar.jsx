...
import { RiBoxingFill, RiLogoutBoxFill } from "react-icons/ri";
...
const SideBar = () => {
	...
	const handleLogout = async () => {
		await axios
			.get("https://hms-backend-deployment-y8nl.onrender.com/api/v1/user/admin/logout", {
		...
		<>
			<nav style={!isAuthenticated ? { display: "none" } : { display: "flex" }} className={show ? "show sidebar" : "sidebar"}>
				<div className="links">
					...
					<RiBoxingFill onClick={handleLogout} />
				</div>
			</nav>
